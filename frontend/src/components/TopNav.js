...
const TopNav = () => {
    ...
    const { user, getUser, leftNavVisibility, setLeftNavVisibility} = useContext(authContext);
    const handleOnClick = () => {
    ...
    useEffect(() => {
        if(localStorage.getItem('auth-token')) getUser();
                        ...
                        <div className="fixed top-0  z-10 flex flex-row items-center justify-between w-full min-w-[100vw] h-14 px-4 md:px-5 dark:bg-black">
                            ...
                            <div className="flex items-center">
                                ...
                                <div className="flex h-8 w-auto overflow-hidden rounded-full md:ml-4 text-white">
                                    <img className='h-8 w-8 rounded-full' src={user?.profilePicture || profileLogo} alt='' />
