...
const NoteAction = (props) => {
    ...
    //add a note
    const addNote = async ({ question, code, description, companyTag, language, link, visibility, important, name }) => {
        try {
            const response = await fetch(`${host}/api/note/addnote`, {
                ...
                body: JSON.stringify({ question, code, description, companyTag, language, link, visibility, important, name })
            })
            const json = await response.json();
            setNote(notes.concat(json.note));
        }
        catch(error){
            console.log({"message": "error occured in NoteAction.js", "error": error})
        }
