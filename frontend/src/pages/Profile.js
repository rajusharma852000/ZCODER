...
const Profile = (props) => {
  ...
  const { getUser, leftNavVisibility, shouldRefresh } = useContext(authContext);

  useEffect(() => {
    if (!localStorage.getItem('auth-token')) navigate('/login');
    const fetchUser = async () => {
      if (user === undefined || shouldRefresh || !localStorage.getItem("zcoder-user")) {
        setLoader(true);
  ...
  const onClickSocialHandles = (event) => {
    const socialHandle = event.currentTarget.getAttribute('name');
    const halfURL = user?.[socialHandle];
            ...
            <div className="flex xs:p-3 flex-col md:flex-row xs:justify-start h-full w-full">
              <div className='relative flex flex-col h-[150px] xs:h-full justify-center items-center mb-3 mt-6  xs:mt-0'>
                <div className='flex w-auto h-auto'>
                  <img className='h-[100px] w-[100px] xs:h-[160px] xs:w-[160px] md:h-[250px] md:w-[250px] xs:mb-4 mb-2 sm:mb-0 rounded-full mx-auto xs:mx-0' src={user?.profilePicture || profileLogo} alt='' />
                </div>
                <span><p className='xs:text-[0.8rem] text-[0.5rem]'>{user?.college}</p></span>
                <span className='xs:text-[0.8rem] mb-2 xs:mb-0 text-[0.5rem]'>{user?.location}</span>
              ...
              <div className=" relative mt-3 flex flex-col xs:ml-8 ml-0 w-[95%] md:w-[calc(100%-200px)] h-auto justify-center items-center xs:items-start">
                <div className='flex items-center justify-center xs:justify-start text-white w-[90%] mb-1 mt-1 bg-orange-600 hover:bg-orange-700 rounded-md flex-wrap'>
                  ...
                  <span className='xs:text-[1rem] text-[0.8rem] pr-1'>{user?.firstName + " " + user?.lastName}</span>
                </div>
                <div className='flex items-center justify-center xs:justify-start text-white w-[90%] mb-1 mt-1 p-1 bg-orange-600 hover:bg-orange-700 rounded-md'>
                  ...
                  <span className='xs:text-[1rem] text-[0.8rem] pr-1'>{user?.email}</span>
                </div>
                {user?.dateOfBirth?.length > 0 && <div className='flex items-center justify-center xs:justify-start text-white w-[90%] mb-1 mt-1 p-1 bg-orange-600 hover:bg-orange-700 rounded-md'>
                  <span className='hidden xs:block font-bold text-[1rem] pl-1 mr-3'>DOB :</span>
                  <span className='xs:text-[1rem] text-[0.8rem] pr-1'>{user?.dateOfBirth?.split('T')[0]}</span>
                </div>}
                <div className='flex items-center justify-center xs:justify-start text-white w-[90%] mb-1 mt-1 p-1 bg-orange-600 hover:bg-orange-700 rounded-md'>
                  <span className='hidden xs:block font-bold text-[1rem] pl-1 mr-3'>Profession :</span>
                  <span className='xs:text-[1rem] text-[0.8rem] pr-1'>{user?.profession}</span>
                </div>
                <div className='flex items-center justify-center xs:justify-start text-white w-[90%] mb-1 mt-1 p-1 bg-orange-600 hover:bg-orange-700 rounded-md'>
                  <span className='hidden xs:block font-bold text-[1rem] pl-1 mr-3'>College/Company :</span>
                  <span className='xs:text-[1rem] text-[0.8rem] pr-1'>{user?.college}</span>
                </div>

                {user?.technicalSkills?.length > 0 && <div className="w-[90%] flex justify-center xs:justify-start h-auto p-1 bg-orange-600 border-orange-600 hover:bg-orange-700 border-2">
